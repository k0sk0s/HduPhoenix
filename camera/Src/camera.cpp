...
bool Mycamera::setVideoHeight(int y)
{
        // 宽高设置时需考虑步进(16)，即设置宽高需16的倍数
        nRet = MV_CC_SetIntValue(handle, "Height",y);   
        if (MV_OK == nRet)
        {
		...
		nRet = MV_CC_SetAOIoffsetY(handle,  96);
	    if (MV_OK == nRet)
        {
				printf("set OffsetXYOK!\n\n");
				return true;
        }
        else
        {
            printf("set OffsetXY failed! nRet [%x]\n\n", nRet);
						return 	false;
        }
}

bool Mycamera::setVideoWidth(int x)
{
        // 宽高设置时需考虑步进(16)，即设置宽高需16的倍数
        nRet = MV_CC_SetIntValue(handle, "Width", x);   
        if (MV_OK == nRet)
        {
            printf("set Width OK!\n\n");
        }
		...
		nRet = MV_CC_SetAOIoffsetX(handle,  64);
	    if (MV_OK == nRet)
        {
...
bool Mycamera::setVideoparam(int x ,int y)
{
	if (!setVideoWidth(x))
	{
		return false;
